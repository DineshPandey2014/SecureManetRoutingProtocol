   ...
   var userData = req.body.userdata;
    ...
    if(userData !== 'undefined' && selectNode == 'NodeTwo') {
        ...
        client.send(message, 0, message.length, PORT, HOST, function (err, bytes) {
            console.log('UDP message sent to node two ' + HOST + ':' + PORT);
        });
    ...
    if (userData !== 'undefined' && selectNode == 'NodeFour') {
        ...
        var message = encrypt(key, String(userData).replace(/^\s+|\s+$/g, ''));
        console.log("User Data" + message);
        clientFour.send(message, 0, message.length, PORTFOUR, HOSTFOUR, function (err, bytes) {
            console.log('UDP message sent to node four ' + HOSTFOUR + ':' + PORTFOUR);
        });
        insertToDB(HOSTFOUR, PORTFOUR);
    }
});
